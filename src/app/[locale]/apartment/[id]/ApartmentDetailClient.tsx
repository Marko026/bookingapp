...
export default function ApartmentDetailClient({
	...
	return (
		<div className="bg-white min-h-screen pb-12 md:pb-24 pt-20 md:pt-28 overflow-x-hidden">
			<div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-20">
				{/* Left Column: Content */}
				<div className="lg:col-span-8">
					{/* Header Section */}
					<div className="mb-6 md:mb-8">
						<h1 className="text-2xl md:text-6xl font-serif font-medium text-gray-900 leading-[1.1] mb-3 md:mb-4">
						...
						<div className="flex flex-wrap items-center gap-3 md:gap-4 text-xs md:text-sm text-gray-500">
							...
							<a
								...
								className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-full font-medium transition-all border border-amber-200 hover:shadow-sm"
							>
					...
					<div className="hidden md:grid grid-cols-4 grid-rows-2 gap-3 h-[500px] mb-12 rounded-[2.5rem] overflow-hidden shadow-sm">
						<button
							type="button"
							className="col-span-2 row-span-2 relative group focus:outline-none focus:ring-4 focus:ring-amber-500/20"
						...
						<button
							type="button"
							className="col-span-1 row-span-1 relative group focus:outline-none focus:ring-4 focus:ring-amber-500/20"
							onClick={() => openGallery(1)}
							aria-label="Open gallery at image 2"
						>
							<Image
						...
						<button
							...
							aria-label="Open gallery at image 4"
						>
							<Image
						...
						>
							...
							<div className="absolute inset-0 bg-black/20 hover:bg-black/30 transition-colors flex items-center justify-center">
								<div className="bg-white/90 backdrop-blur-md px-5 py-2.5 rounded-full text-sm font-bold shadow-lg flex items-center gap-2 hover:scale-105 transition-transform text-gray-900">
					...
					{/* Mobile Image Slider */}
					<div className="md:hidden relative h-[35vh] rounded-[1.5rem] overflow-hidden mb-4 shadow-lg">
						<Image
					...
					{/* Mobile "Book Now" Quick Action */}
					<div className="lg:hidden mb-8">
						<Button
							onClick={() => setIsMobileBookingOpen(true)}
							className="w-full py-5 md:py-8 text-lg md:text-xl bg-amber-600 text-white hover:bg-amber-700 shadow-2xl shadow-amber-600/20 rounded-2xl flex items-center justify-center gap-3 font-bold"
						>
							{t("reserveButton")}
					...
					<div className="flex flex-col gap-8 md:gap-10">
						<div className="grid grid-cols-3 gap-2 py-4 md:py-6 border-y border-gray-100">
							<div className="flex flex-col items-center text-center gap-1">
								...
								<p className="text-[10px] text-gray-500">{t("capacity")}</p>
							</div>
							<div className="flex flex-col items-center text-center gap-1">
								<div className="p-2 bg-amber-50 text-amber-600 rounded-full">
									<BedDouble size={16} />
								...
								<p className="text-[10px] text-gray-500">{t("sleeping")}</p>
							</div>
							<div className="flex flex-col items-center text-center gap-1">
								<div className="p-2 bg-amber-50 text-amber-600 rounded-full">
									<ShieldCheck size={16} />
								...
								<p className="text-[10px] text-gray-500">{t("premium")}</p>
							</div>
						</div>
				...
				<div className="hidden lg:block lg:col-span-4 relative z-10">
			...
			<Dialog open={isMobileBookingOpen} onOpenChange={setIsMobileBookingOpen}>
				<DialogContent
					...
					className="w-full max-w-sm sm:max-w-[95%] p-0 border-none rounded-2xl"
				>
					<div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-h-[85vh] overflow-y-auto">
						...
						<div className="grid grid-cols-2 gap-2 sm:gap-3">
							<div
								className={`p-3 sm:p-4 rounded-2xl border transition-all ${dateRange?.from ? "bg-amber-50 border-amber-200" : "bg-gray-50 border-gray-100"}`}
							>
								<p className="text-[10px] sm:text-xs uppercase font-bold text-gray-400 tracking-wider mb-1">
									{t("checkIn")}
								</p>
								<p
									className={`font-medium text-sm sm:text-base ${dateRange?.from ? "text-amber-900" : "text-gray-400"}`}
								>
									{dateRange?.from
										? dateRange.from.toLocaleDateString()
										: t("addDate")}
							...
							<div
								className={`p-3 sm:p-4 rounded-2xl border transition-all ${dateRange?.to ? "bg-amber-50 border-amber-200" : "bg-gray-50 border-gray-100"}`}
							>
								<p className="text-[10px] sm:text-xs uppercase font-bold text-gray-400 tracking-wider mb-1">
									{t("checkOut")}
								</p>
								<p
									className={`font-medium text-sm sm:text-base ${dateRange?.to ? "text-amber-900" : "text-gray-400"}`}
								>
									{dateRange?.to
										? dateRange.to.toLocaleDateString()
										: t("addDate")}
						...
						{/* Calendar Container */}
						<div className="bg-stone-50 border border-stone-200 rounded-2xl p-2 sm:p-3 shadow-inner">
							<div className="flex justify-center">
								<BookingCalendar
									apartmentId={apartment.id}
									onDateSelect={setDateRange}
						...
						{dateRange?.from && dateRange?.to ? (
							<div className="space-y-4">
								...
								<div className="flex flex-col gap-3">
									<Button
										...
										className="py-3 sm:py-4 text-sm sm:text-base font-semibold bg-amber-600 text-white hover:bg-amber-700 shadow-lg shadow-amber-600/25 rounded-2xl transition-all"
									>
										{t("reserveButton")}
									</Button>
									<Button
										onClick={() => setIsMobileBookingOpen(false)}
										className="py-3 sm:py-4 text-sm sm:text-base font-semibold text-gray-600 bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 rounded-2xl transition-all"
									>
										{t("cancelButton") || "Otkaži"}
									</Button>
								</div>
							</div>
						) : (
							<div className="space-y-4">
								...
								<Button
									onClick={() => setIsMobileBookingOpen(false)}
									className="w-full py-3 sm:py-4 text-sm sm:text-base font-semibold text-gray-600 bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 rounded-2xl transition-all"
								>
									{t("cancelButton") || "Otkaži"}
								</Button>
							</div>
